...
def fetch_and_write(ticker, contract_size, friction, start, end, interval, file_handle):
    ...
    try:
        ...
        formatted_dates = df['timestamp'].dt.strftime('%Y-%m-%d')
        output_block = (
            f"Stock: {ticker}\n"
            f"ContractSize:\n{contract_size}\n"
...
if __name__ == "__main__":
    ...
    with open("OfferBid_SellPriceDataDailyData.txt", "w") as file_handle:
    ...
    print("All downloads complete and saved to OfferBid_SellPriceDataDailyData.txt")
