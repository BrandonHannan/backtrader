...
class CustomChannelBreakout: private TradingStrategy{
    ...
    public:
        ...
        TradingStrategy(bal, cOP, pos, cPoses), lookbackPeriod(lbPeriod), priceVolatilityThreshold(pVT),
        volumeVolatilityThreshold(vVT), volumeDropThreshold(vDT), waitingPeriod(wPeriod), volumeComparison(vC),
        volumeDropComparison(vDC), priceSurge(pS), volumeComparisonPriceSurge(vCPS) {}
};